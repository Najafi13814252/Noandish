<template>
    <Teleport to="body">
        <section class="fixed inset-0 flex items-center justify-center z-50 bg-black/50 backdrop-blur-xs font-dana">
            <div class="relative border-2 border-teal-100 bg-white md:rounded-2xl shadow-lg flex justify-center w-full md:w-[30rem] h-full md:h-[35rem]">
                <Icon class="text-3xl absolute right-2 top-2 text-gray-800 cursor-pointer" name="solar:close-circle-outline" @click="$emit('closeLogin')"/>
                <!-- forms -->
                <div class="rounded-2xl text-center">
                    <div class="flex flex-col items-center gap-4 my-2">
                        ...
                        <div class="flex flex-col gap-2 mb-4">
                            <span class="text-main-100 text-4xl font-lalezar">{{ titleForm }}</span>
                            <p class="text-gray-400 text-lg">{{ descriptionForm }}</p>
                        </div>
                    </div>
                    <form class="flex flex-col items-center mx-auto gap-6 w-fit">
                        <input class="w-96 p-2 bg-white rounded border border-teal-500" type="text"
                            placeholder="نام کاربری را وارد کنید">
                        <input class="w-96 p-2 bg-white rounded border border-teal-500" type="password"
                    ...
                    <div class="mt-4">
                        <span class="mt-4 text-sm text-gray-800 hover:text-sky-600 duration-200 cursor-pointer"
