<template>
    <section class="border border-gray-200 bg-white p-4 rounded-lg flex flex-col gap-6 text-gray-800">
        <span class="text-xl">جلسات دوره</span>

        <div class="flex flex-col gap-6">
            <!-- course box -->
            <div class="border border-gray-200 p-4 rounded-md" v-for="(item, index) in filters" :key="index">
                <!-- main title course -->
                ...
                <div class="my-8 flex flex-col gap-6" v-if="openIndex === index">
                    <div class="flex items-center justify-between p-4 rounded-lg shadow cursor-pointer hover:bg-gray-50 duration-200" v-for="(option, index) in item.options" :key="index">
                        <div class="flex items-center gap-2">
                            <div class="flex items-center gap-2">
                        ...
                        <div class="flex items-center bg-green-100 p-2 rounded-full" v-if="option.type === 'public'">
                            <Icon class="text-green-500 text-xl" name="solar:eye-outline" />
                        </div>
                        <div class="flex items-center bg-fuchsia-100 p-2 rounded-full" v-else>
